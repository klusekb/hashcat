 ...
 * License.....: MIT
 *
 * Mode 23801: RAR3-p (Signature)
 *
 * Key derivation identical to 23800.
 * hook23: finalize key, decrypt first block, pass to CPU hook.
 * comp: check signature_matched from CPU hook.
 * For stored files, comp also does direct signature check (no hook needed).
 */
...
/**
 * Init kernel — identical to 23800
 */

KERNEL_FQ KERNEL_FA void m23801_init (KERN_ATTR_TMPS_HOOKS_ESALT (rar3a_tmp_t, rar3a_hook_t, rar3a_t))
{
...
/**
 * Loop kernel — identical to 23800
 */

KERNEL_FQ KERNEL_FA void m23801_loop (KERN_ATTR_TMPS_HOOKS_ESALT (rar3a_tmp_t, rar3a_hook_t, rar3a_t))
{
  const u64 gid = get_global_id (0);
...
/**
 * Hook23 kernel (GPU side) — finalize key, decrypt first block,
 * pass key/iv/decrypted block to CPU hook via hooks buffer.
 * Identical to 23800's hook23 kernel.
 */

KERNEL_FQ KERNEL_FA void m23801_hook23 (KERN_ATTR_TMPS_HOOKS_ESALT (rar3a_tmp_t, rar3a_hook_t, rar3a_t))
{
  const u64 gid = get_global_id (0);
  ...
  /* Initialize signature_matched to 0 — CPU hook will set it */
  hooks[gid].signature_matched = 0;
}

/**
 * Comp kernel — check if CPU hook found a signature match
 */

KERNEL_FQ KERNEL_FA void m23801_comp (KERN_ATTR_TMPS_HOOKS_ESALT (rar3a_tmp_t, rar3a_hook_t, rar3a_t))
{
  const u64 gid = get_global_id (0);

  if (gid >= GID_CNT) return;

  if (hooks[gid].signature_matched != 1) return;

  const u32 r0 = 1;  /* matches digest[0] = 1 set by parser */
  const u32 r1 = 0;
  const u32 r2 = 0;
  const u32 r3 = 0;
